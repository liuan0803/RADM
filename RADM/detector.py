...
class RADM(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.num_classes = cfg.MODEL.RADM.NUM_CLASSES
        self.num_proposals = cfg.MODEL.RADM.NUM_PROPOSALS
        self.hidden_dim = cfg.MODEL.RADM.HIDDEN_DIM
        self.num_heads = cfg.MODEL.RADM.NUM_HEADS
        ...
        # build RADM
        timesteps = 1000
        sampling_timesteps = cfg.MODEL.RADM.SAMPLE_STEP
        ...
        self.scale = cfg.MODEL.RADM.SNR_SCALE
        self.box_renewal = True
        self.use_ensemble = True

        #self.p_enc_2d = PositionalEncodingPermute2D(3)
        ...
        # Loss parameters:
        class_weight = cfg.MODEL.RADM.CLASS_WEIGHT
        giou_weight = cfg.MODEL.RADM.GIOU_WEIGHT
        l1_weight = cfg.MODEL.RADM.L1_WEIGHT
        no_object_weight = cfg.MODEL.RADM.NO_OBJECT_WEIGHT
        self.deep_supervision = cfg.MODEL.RADM.DEEP_SUPERVISION
        self.use_focal = cfg.MODEL.RADM.USE_FOCAL
        self.use_fed_loss = cfg.MODEL.RADM.USE_FED_LOSS
        self.use_nms = cfg.MODEL.RADM.USE_NMS
    ...
    def forward(self, batched_inputs, do_postprocess=True):
        ...
        #enc_output = self.dropout(self.p_enc_2d(images.tensor))
        ...
        text_features = [x["text_fea"]['feats'].to(self.device) for x in batched_inputs]
        text_features = torch.cat(text_features, dim=0).unsqueeze(0)
        txt_mask = torch.stack([x["text_mask"].to(self.device) for x in batched_inputs], dim=0)
        # Prepare Proposals.
        if not self.training:
            results = self.ddim_sample(batched_inputs, features, images_whwh, images, text_features, txt_mask)
            return results
    ...
    def prepare_targets(self, targets):
        ...
        for i, targets_per_image in enumerate(targets):
            ...
            batch_ind=torch.tensor([i for x in range(targets_per_image.gt_boxes.tensor.shape[0])], dtype=torch.int, device=self.device).reshape(-1,1)
            # target_for_top.append(torch.cat((batch_ind, targets_per_image.gt_boxes.tensor), dim=1))
        ...
        if self.use_focal or self.use_fed_loss:
            ...
            for i, (scores_per_image, box_pred_per_image, image_size) in enumerate(zip(
                ...
                if self.use_nms:
                    ...
                    scores_per_image = scores_per_image[keep]
                    labels_per_image = labels_per_image[keep]
                    box_pred_per_image = box_pred_per_image[torch.where(scores_per_image > self.threshold)]
                    scores_per_image = scores_per_image[torch.where(scores_per_image > self.threshold)]
                    labels_per_image = labels_per_image[torch.where(scores_per_image > self.threshold)]
                result.pred_boxes = Boxes(box_pred_per_image)
                result.scores = scores_per_image
                result.pred_classes = labels_per_image
            ...
            for i, (scores_per_image, labels_per_image, box_pred_per_image, image_size) in enumerate(zip(
                ...
                if self.use_nms:
                    ...
                    labels_per_image = labels_per_image[keep]
                result = Instances(image_size)
                result.pred_boxes = Boxes(box_pred_per_image[:5])
                result.scores = scores_per_image[:5]
                result.pred_classes = labels_per_image[:5]
                results.append(result)
